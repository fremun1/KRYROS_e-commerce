...
export default function TopSellingSection() {
  ...
  const [sectionTitle, setSectionTitle] = useState("Top Selling Items");
  ...
  useEffect(() => {
    Promise.all([
      ...
      fetchHomepageSections("TopSelling"),
    ]).then(([prods, sections]) => {
      setProducts(prods.slice(0, 12));

      const cms = sections.find((s: ApiHomepageSection) => s.type === "TopSelling");
      ...
      {loading ? (
        <div className={`flex gap-2 ${scroll ? "overflow-hidden" : ""}`}>
          {Array.from({ length: 4 }).map((_, i) => (
            <div key={i} className={`shrink-0 w-[calc(50vw-16px)] md:w-full ${scroll ? "" : "md:grid-cols-4"}`}
              style={{ aspectRatio: scroll ? "3/4" : "auto" }} />
          ))}
        </div>
      ) : (
