...
export default function TopExpressSection() {
  const [products, setProducts] = useState<Product[]>([]);
  const [loading, setLoading] = useState(true);
  const [sectionTitle, setSectionTitle] = useState("Top Express");
  ...
  useEffect(() => {
    Promise.all([
      fetchProducts({ popularity: "trending", take: 12 }),
      fetchHomepageSections("TopExpress"),
    ]).then(([prods, sections]) => {
      setProducts(prods.slice(0, 12));

      const cms = sections.find((s: ApiHomepageSection) => s.type === "TopExpress");
